 ...
 *
 * djvu.c - functions to load from single-page DjVuBitonal files
 *
...
MDJVU_IMPLEMENT void mdjvu_locate_jb2_chunk(mdjvu_file_t file)
{
    FILE *f = (FILE *) file;
    ...
    {
        fprintf(stderr, "Corrupted IFF header\n");
        exit(1);
    }
    ...
    {
        fprintf(stderr, "Unable to find FORM chunk\n");
        exit(1);
    }

    if (read_uint32_most_significant_byte_first(f) != ID_DJVU)
    {
        fprintf(stderr, "Wrong DjVu file type (one-page Bitonal/Compound required)\n");
        exit(1);
    }
    ...
    {
        fprintf(stderr, "Unable to find Sjbz chunk\n");
        exit(1);
    }
}

MDJVU_FUNCTION mdjvu_image_t mdjvu_load_from_djvu_file(mdjvu_file_t file)
{
    mdjvu_locate_jb2_chunk(file);
    return mdjvu_load_from_jb2_file(file);
}

MDJVU_IMPLEMENT mdjvu_image_t mdjvu_load_from_djvu(const char *path)
{
    mdjvu_image_t result;
    FILE *f = fopen(path, "rb");
    if (!f) return NULL;
    result = mdjvu_load_from_djvu_file((mdjvu_file_t) f);
    fclose(f);
    return result;
}
